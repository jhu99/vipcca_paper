...
plot_figures_for_germline<-function(part="female"){
 ...
 result_dir<-c("uncorrected","desc","harmony","liger","mnn","scanorama","seurat",vipcca_res)
  ...
  for(i in 1:length(result_dir)){
  	...
  	g1<-DimPlot(datasets.integrated,group.by = c("week"),pt.size=10,
  	            order=c("4W","5W","7W","8W","10W","11W","12W","14W",
  											"17W","18W","20W","23W","24W","26W"))+ggtitle(method_names[i])
  ...
  legend_1<-cowplot::get_legend(g1list[[1]]+guides(col=guide_legend("Week"))+
  ...
  print(gc)
  dev.off()
  saveRDS(list(p1arow,p1brow,legend_1,legend_2,g3,g4,gc,gmarker),file = paste0("./results/germline/",part,"/ggplots.rds"))
...
plot_main_figure<-function(plot_germ,part="male"){
  ...
  plot(plot_grid(g1,g2,g3,g4,ncol = 1,rel_widths = c(1,1),labels = c("","","","e"),label_size = 60))
  dev.off()
}
...
png("./results/germline/figures_female.png",width = 60,height = 80, units = "in", res = 100)
plot_female[[2]]/plot_female[[4]]/plot_female[[1]]/plot_female[[3]]/(plot_female[[5]]+plot_female[[6]]+plot_layout(nrow = 1))/plot_female[[8]]+plot_layout(heights = c(5,1,5,1,4,4))+plot_annotation(tag_levels = 'A')
dev.off()


plot_male[[7]]/plot_female[[7]]
