...
sil_scxx_cvae <- silhouette(as.integer(datasets.integrated[[celltype_col]][[1]]),dist = dist(datasets.integrated@reductions$scxx@cell.embeddings))
...
sil_scxx_cvae2 <- silhouette(as.integer(datasets.integrated[[celltype_col]][[1]]),dist = dist(datasets.integrated@reductions$scxx@cell.embeddings))
...
sil_scanorama <- silhouette(as.integer(datasets.integrated[[celltype_col]][[1]]),dist = dist(datasets.integrated@reductions$scanorama@cell.embeddings))
...
sil_desc <- silhouette(as.integer(datasets.integrated[[celltype_col]][[1]]),dist = dist(datasets.integrated@reductions$Embeded_z0.45@cell.embeddings))
png(paste0(result_path,"desc/desc_dataset_umap.png"),width=8, height=8, units="in",res=150)
p13 <- DimPlot(datasets.integrated, reduction = "umap", group.by = "batch")+ NoLegend()+ ggtitle("desc")
plot(p13)
dev.off()
png(paste0(result_path,"desc/desc_celltype_umap.png"),width=8, height=8, units="in",res=150)
p14 <- DimPlot(datasets.integrated, reduction = "umap", group.by = "cell.type", label = TRUE, 
...
sil_seurat <- silhouette(as.numeric(as.factor(datasets.integrated[[celltype_col]][[1]])),dist = dist(datasets.integrated@reductions$pca@cell.embeddings))
...
datasets.integrated <- RunUMAP(scAlignSeuratObj, reduction = "ALIGNED-CCA", dims = 1:30)
datasets.integrated <- FindNeighbors(datasets.integrated, dims = 1:30, reduction = "ALIGNED-CCA")
datasets.integrated <- FindClusters(datasets.integrated, resolution = 0.45)
ari_scalign <- adjustedRandIndex(x=as.integer(datasets.integrated$seurat_clusters),y=datasets.integrated[[celltype_col]][[1]])
sil_scalign <- silhouette(as.numeric(as.factor(datasets.integrated[[celltype_col]][[1]])),dist = dist(datasets.integrated@reductions$`ALIGNED-CCA`@cell.embeddings))
...
sil_mnn <- silhouette(as.integer(as.factor(datasets.integrated[[celltype_col]][[1]])),dist = dist(datasets.integrated@reductions$pca@cell.embeddings))
...
plot_grid(p1,p3,p5,p7,p9,p11,p13, labels = c("a","b","c","d","e","f","g"))
dev.off()
png(paste0(result_path,"celltype_umap.png"),width=15, height = 15, units="in", res=100)
plot_grid(p2,p4,p6,p8,p10,p12,p14, labels = c("a","b","c","d","e","f","g"))
dev.off()
png(paste0(result_path,"cell_alignment_umap.png"),width=28, height = 8, units="in", res=100)
plot_grid(p1,p3,p5,p7,p9,p11,p13,p2,p4,p6,p8,p10,p12,p14, labels = c("a","b","c","d","e","f","g","h","i","j","k","l","m","n"),nrow=2)
dev.off()
# 
# 
...
boxplot(sil~group,data=data, ylab="Silhouette Information",las=2)
dev.off()
