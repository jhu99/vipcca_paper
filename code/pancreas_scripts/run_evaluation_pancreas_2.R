...
g1row<-plot_grid(g1row,legend1,rel_widths = c(4,1.0))
...
g2row<-plot_grid(g2row,legend2,rel_widths = c(4,1.0))
...
g1<-ggplot(data = ari_data, aes(y=ari_celltype,x=group))+
	...
	theme(axis.title.y = element_text(size=60),axis.text.y = element_text(size=60),
...
g4<-ggplot(data = df,aes(x=aa,y=mm))+
	geom_bar(stat="identity",fill="orange")+
	ylab("Mixing Metric")+theme_bw(base_size=40)+
	theme(axis.title.x = element_blank(),
...
# 	geom_boxplot()+
# 	ylab("kBET (acceptance rate)")+theme_bw(base_size=30)+
# 	theme(axis.title.x = element_blank())

g5<-ggplot(data = kbet_data, aes(x=neighbor_size,y=mkb))+
	geom_line(aes(group=group,color=group),size=2)+ylab("kBET")+
	xlab("% sample size")+theme_bw(base_size=40)+
	theme(legend.title = element_blank(),legend.text = element_text(size = 50),
				axis.text = element_text(size = 60),
				axis.title = element_text(size = 60),legend.position = c(0.3,0.5))+
	guides(colour = guide_legend(override.aes = list(size=10)))
...
g6<-ggplot(br.all, aes(y=log_p_val,x=ctype,fill=method))+
	...
	labs(y="-log (p_value)")+
	theme(axis.title.x = element_blank())+
	theme(legend.title =element_blank(),axis.text.y = element_text(60),
				legend.text = element_text(size = 60),legend.position = c(0.4,0.5))
...
g3row<-plot_grid(plotlist = list(g4,g5,legend3),
								 labels=c("c","d",""),nrow=1,label_size = 60,rel_widths = c(2.5,1.5,1))

g4row<-plot_grid(plotlist = list(g1,g6,legend4),
								 labels=c("e","f",""),ncol=3,label_size = 60,rel_widths = c(2.5,1.5,1))

png(paste0(result_path,"all_in_one.png"),width = 45,height = 55,units = "in",res = 100)
plot_grid(plotlist = list(g1row,g2row,g3row,g4row),ncol = 1,rel_heights = c(2,2,1,1),labels = c("a","b",""),label_size = 60)
dev.off()
